...
<div id="wizard-wrapper" <?php echo wpsstm_get_classes_attr($classes);?>>
    ...
    //reset wizard
    if ( wpsstm_is_backend() && wpsstm_wizard()->tracklist->feed_url ){
        
        $input_id = 'wpsstm_wizard_reset';

        printf(
            '<small><input id="%s" type="checkbox" name="%s[reset]" value="on" %s /><label for="%s" class="wizard-field-desc">%s</label></small>',
            $input_id,
            'wpsstm_wizard',
    ...
    //restore wizard backup
    if ( wpsstm_is_backend() && wpsstm_wizard()->tracklist->has_wizard_backup() ){
        
        $input_id = 'wpsstm_wizard_restore';

        printf(
            '<small><input id="%s" type="checkbox" name="%s[restore]" value="on" %s /><label for="%s" class="wizard-field-desc">%s</label></small>',
            $input_id,
            'wpsstm_wizard',
            checked($reset_checked, true, false),
            $input_id,
            __('Restore previous wizard settings','wpsstm')
        );
    }
