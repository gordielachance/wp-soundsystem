...
class WPSSTM_Soundcloud_User_Api_Preset{
    ...
    function can_handle_url($url){
        $user_slug = $this->get_web_user_slug($url);
        if ( !$user_slug ) return;
        return true;
    }
    
    function is_api_url($url){
        $pattern = '~^http(?:s)?://api.soundcloud.com/(.*)~i';
        preg_match($pattern,$url, $matches);
        return ( !empty($matches) );
    }
    
    function get_web_user_slug($url){
        $pattern = '~^http(?:s)?://(?:www\.)?soundcloud.com/([^/]+)~i';
        preg_match($pattern,$url, $matches);
        return isset($matches[1]) ? $matches[1] : null;
    }
    
    function get_web_user_page($url){
        $pattern = '~^http(?:s)?://(?:www\.)?soundcloud.com/[^/]+/([^/]+)~i';
        preg_match($pattern,$url, $matches);
        return isset($matches[1]) ? $matches[1] : null;
    }
    
    function get_api_user_id($user_slug){
    ...
    function web_to_api_url($url){
        ...
        if ( $user_slug = $this->get_web_user_slug($url) ){

            if ( !$user_id = $this->get_api_user_id($user_slug) ){
                return new WP_Error( 'wpsstm_soundcloud_missing_user_id', __('Required user ID missing.','wpsstm') );
            }

            $api_page = null;
            $page_slug = $this->get_web_user_page($url);
            $client_id = $wpsstm_souncloud->get_options('client_id');
    ...
    function get_remote_title($title,$remote){
        ...
        if ( $this->can_handle_url($remote->url) ){
            ...
            $user_slug = $this->get_web_user_slug($remote->url);
            $page_slug = $this->get_web_user_page($remote->url);
